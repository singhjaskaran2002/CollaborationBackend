...
public class FriendDAOImplementation implements FriendDAO 
{
	...
	/*public List<User> suggestedPeopleList(String useremail) {
		List<String> users = sessionFactory.getCurrentSession().createSQLQuery("select username from c_user where username not in(select friendname from c_friend where ='"+useremail+"' OR friendname='"+loginname+"') and username!='"+loginname+"'").list();
